...
def process_nlu_result(model_name: str, outpath: str, full_model_name=None):
    ...
    for task_type in results.keys():
        ...
        data = {
            "config": {
                "model_name": (
                    full_model_name if full_model_name is not None else model_name
                ),
            },
...
if __name__ == "__main__":
    ...
    has_success = False
    status = process_nlu_result(
        model_name, args.result_path, full_model_name=full_model_name
    )
    if status:
        has_success = True
    status = process_nlg_result(
        model_name, args.result_path, full_model_name=full_model_name
    )
    if status:
        has_success = True
    status = process_llm_result(
        model_name, args.result_path, full_model_name=full_model_name
    )
    if status:
        has_success = True
