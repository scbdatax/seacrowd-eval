...
def extract():
    ...
    for config_name in NLG_TASK_LIST + NLU_TASK_LIST:
        schema = config_name.split("_")[-1]
        con = nc_conhelp.for_config_name(config_name)
        cfg_name_to_dset_map[config_name] = (config_name, list(con.tasks)[0].value)
    with open("cfg_name_to_dset_map.json", "w") as w:
        json.dump(cfg_name_to_dset_map, w)
